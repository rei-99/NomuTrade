...
**Decision D-11:** The replay **loops the dataset**: minute bars are replayed in dataset-time order at `REPLAY_BARS_PER_SECOND` (default 1.0 ≈ 6.5 min per market day, emitted on wall-second boundaries); each pass starts at `REPLAY_START` (empty = first bar; ISO date/datetime selects a later start); `REPLAY_MODE=loop|hold` (default `loop`).
