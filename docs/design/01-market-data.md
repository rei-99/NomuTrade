...
- **Replayer** (`worker.py`) — walks the stored 1-minute bars in dataset-time order at `REPLAY_BARS_PER_SECOND` unique timestamps per second (default **1.0 ≈ 6.5 min per market day**, paced on a wall-clock-aligned grid), publishing all instruments' bars sharing a timestamp as `market.ticks` events that carry the *dataset* timestamp. Tick payload: `{instrument_id, symbol, ts, price, open, high, low, close, volume}` — `price` is the bar close; OHLC/volume are running day aggregates from the registry snapshot. At the end of the dataset it loops to the start (`REPLAY_MODE=loop`, default) or goes idle holding last prices (`hold`). Fallback mode (no `data/`): random-walk ticks at `TICK_INTERVAL_MS` with throttled persistence — the pre-dataset behavior.
